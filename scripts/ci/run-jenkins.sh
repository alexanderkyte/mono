...
if [[ ${label} == w* ]] || [[ ${label} == 'debian-ppc64el' ]] || [[ ${label} == 'centos-s390x' ]];
    ...
    EXTRA_CONF_FLAGS="$EXTRA_CONF_FLAGS --with-monodroid=no --with-monotouch=no --with-monotouch_watch=no --with-monotouch_tv=no --with-xammac=no --with-sgen-default-concurrent=yes"
