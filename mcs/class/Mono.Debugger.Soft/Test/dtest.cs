...
public class DebuggerTests
{
	...
	BreakpointEvent run_until (string name) {
		...
		Assert.IsNotNull (m);
		Console.WriteLine ("X: " + name + " Offsets: ");
		foreach (var off in m.ILOffsets)
		...
		while (true) {
			...
			Console.WriteLine ("Stepped one, {0}", frame.il_offset);
	...
	StepEventRequest create_step (Event e) {
		var frame = e.Thread.GetFrames () [0];
		Console.WriteLine ("Create step at IL of {0}", frame.il_offset);
	...
	Event step_once () {
		...
		var frame = e.Thread.GetFrames () [0];
		Console.WriteLine ("We have an IL of {0}", frame.il_offset);
		return e;
	}
	...
	public void Locals () {
		...
		// Test that locals are initialized
		var frame = e.Thread.GetFrames () [0];
		Console.WriteLine ("After skipping the nop, we have an IL of {0}", frame.il_offset);
