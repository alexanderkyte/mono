...
class MakeBundle {
	...
	static int Main (string [] args)
	{
		...
		for (int i = 0; i < top; i++){
			...
			default:
				if (args [i][0] == '-')
					throw new Exception (String.Format ("Parsed option as assembly {0}", args [i]));
				sources.Add (args [i]);
				break;
			}
		}
	...
	class PackageMaker {
		...
		//public int AddAotFile (string fname)
		//{
			//int ret = 0;
			//string aot_out = String.Format ("{0}.aot_out", fname);
			//Console.WriteLine ("Added {0}", aot_out);
			//using (Stream fileStream = File.OpenRead (aot_out)) {
				//ret = (int) fileStream.Length;
				//if (!quiet)
					//Console.WriteLine ("Adding AOT at {0:x} with input {1}", package.Position, fileStream.Length);
...
");
			...
			else
			{
				...
				if (static_link)
				{
					...
					var aot_libs = new StringBuilder ("");
					foreach (var apath in aot_paths) {
						aot_libs.Append (apath);
						aot_libs.Append (" ");
					}
