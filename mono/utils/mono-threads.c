...
mono_thread_info_set_inited (void)
{
	...
	if (old == MONO_END_INIT_CB) {
		g_warning ("Global threads inited twice");
		return;
	}
...
static gboolean
utility_thread_handle_inbox (MonoUtilityThread *thread)
{
	UtilityThreadQueueEntry *entry = (UtilityThreadQueueEntry *) mono_lock_free_queue_dequeue (&thread->work_queue);
	if (!entry)
		return FALSE;
	
	thread->callbacks.command (thread->state_ptr, &entry->payload);
...
utility_thread (void *arg)
{
	...
	while (mono_atomic_load_i32 (&thread->run_thread)) {
		...
		MONO_EXIT_GC_SAFE;
		utility_thread_handle_inbox (thread);
	}

	/* Drain any remaining entries on shutdown. */
	while (utility_thread_handle_inbox (thread));
...
MonoUtilityThread *
mono_utility_thread_launch (size_t payload_size, MonoUtilityThreadCallbacks *callbacks)
{
	...
	mono_lock_free_allocator_init_allocator (&thread->message_allocator, &thread->message_size_class, MONO_MEM_ACCOUNT_OTHER);
