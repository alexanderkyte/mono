...
emit_llvm_file (MonoAotCompile *acfg)
{
	...
	if (acfg->aot_opts.llvm_only) {
		...
		// FIXME: arch
		command = g_strdup_printf ("clang++ -fexceptions -march=x86-64 -fpic -msse -msse2 -msse3 -msse4 -O0 -fno-optimize-sibling-calls -Wno-unknown-warning-option -Wno-override-module -c -o \"%s\" \"%s.opt.bc\"", acfg->llvm_ofile, acfg->tmpbasename);
