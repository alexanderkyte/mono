...
static void
mono_setup_dedup_state (MonoAotCompile *acfg, gpointer *aot_state, MonoAssembly *ass, MonoAotState **astate, gboolean *is_dedup_dummy) 
{
	// fills out acfg->dedup_cache
	if (acfg->aot_opts.dedup) {
		mono_read_method_cache (acfg);
	} else if (aot_state && acfg->aot_opts.dedup_include) {
