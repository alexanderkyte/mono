...
static void
clean_contexts (MonoRelationsEvaluationContextContainer *contexts, int number)
{
	memset(contexts->statuses, number * sizeof(MonoRelationsEvaluationStatus), MONO_RELATIONS_EVALUATION_NOT_STARTED);
}
...
{
	MonoRelationsEvaluationContext * const context = &(area->contexts.contexts [variable]);
	MonoRelationsEvaluationStatus * const status = &(area->contexts.statuses [variable]);
		...
		while (relation != NULL) {
			...
			case MONO_VARIABLE_SUMMARIZED_VALUE:
				...
				if (relation->relation != MONO_ANY_RELATION) {
					...
					MonoRelationsEvaluationContext *related_context = &(area->contexts.contexts [related_variable]);
					MonoRelationsEvaluationStatus related_status = area->contexts.statuses [related_variable];
				...
				for (phi = 0; phi < relation->related_value.value.phi.number_of_alternatives; phi++) {
					...
					if (*status & MONO_RELATIONS_EVALUATION_IS_RECURSIVE) {
						...
						*status = MONO_RELATIONS_EVALUATION_IN_PROGRESS;
						...
						MONO_RELATIONS_EVALUATION_RANGES_UNION (phi_ranges, area->contexts.contexts [phi_alternative].ranges);
					}
				}
	...
	case MONO_RELATIONS_EVALUATION_IN_PROGRESS: {
		...
		if (evaluation_is_definition) {
			...
			while (current_context != last_context) {
				int index = father_context - area->contexts.contexts;
				MonoRelationsEvaluationStatus *current_status = &(area->contexts.statuses [index]);
...
remove_abc_from_inst (MonoInst *ins, MonoVariableRelationsEvaluationArea *area)
{
	...
	MonoRelationsEvaluationContext *array_context = &(area->contexts.contexts [array_variable]);
	MonoRelationsEvaluationContext *index_context = &(area->contexts.contexts [index_variable]);
...
{
	MonoRelationsEvaluationContext *context = &(area->contexts.contexts [reg]);
...
mono_perform_abc_removal (MonoCompile *cfg)
{
	...
	area.contexts.contexts = (MonoRelationsEvaluationContext *)
		mono_mempool_alloc0 (cfg->mempool, sizeof (MonoRelationsEvaluationContext) * (cfg->next_vreg));

	area.contexts.statuses = (MonoRelationsEvaluationStatus *)
