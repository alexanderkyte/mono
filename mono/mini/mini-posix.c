...
static GHashTable thread_snapshots;

GHashTable
mono_snapshot_threads (void)
{
	thread_snapshots = g_hash_table_new ();


	GHashTable ret = thread_snapshots;
}

static void 
add_to_thread_snapshots (intptr_t thread_id, GPointerArray traces)
{
	g_assert (thread_snapshots);
	g_hash_table_insert (thread_id, traces);
}
...
typedef struct
{
	gboolean managed;
	union {
		intptr_t native_address;
		struct {
			MonoAssembly *managed_assembly;
			unsigned int managed_token    : 31;
		};
	};
} MonoDiscreetMethodRef;

static void 
add_frame_to_stacktrace (StackFrameInfo *frame, MonoContext *ctx, gpointer data)
{
	GPtrArray *stacktrace = (GPtrArray *)data;
	intptr_t fn_ptr = mono_arch_ip_from_context (ctx)
	MonoJitInfo *ji = mono_jit_info_table_find_internal (mono_domain_get (), mono_arch_ip_from_context (ctx), TRUE, TRUE);

	MonoDiscreetMethodRef *summary = g_new0 (sizeof (MonoDiscreetMethodRef));

	if (frame->type == FRAME_TYPE_MANAGED)
	else
		summary->native_address = fn_ptr;

	g_ptr_array_add (stacktrace, summary);
}

MONO_SIG_HANDLER_FUNC (static, synch_snapshot)
{
	MONO_SIG_HANDLER_INFO_TYPE *info = MONO_SIG_HANDLER_GET_INFO ();
	MonoContext ctx = mono_sigctx_to_monoctx (sigctx, &ctx);

	// This signal handler is going to be running in a very
	// specific situation. When we encounter a native crash, we are going to
	// send this signal to all crashing processes and their child processes 
	// (besides the debugger thread)
	// These child processes are going to fill out their data in a hash table.
	gpointer stack_walk_data = (gpointer) g_ptr_array_new ();
	gpointer stack_walk_func ;

	mono_walk_stack_with_ctx (add_frame_to_stacktrace, &ctx, MONO_UNWIND_LOOKUP_ALL, stack_walk_data);
